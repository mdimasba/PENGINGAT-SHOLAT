...
let checkInterval;
let statusBarInterval;
let dailyRefreshTimeout;
let currentPanel;
let triggeredPrayers = new Set();
let triggeredPreReminders = new Set();
...
function activate(context) {
    ...
    const openSettingsCmd = vscode.commands.registerCommand('islamicReminder.openSettings', () => {
        vscode.commands.executeCommand('workbench.action.openSettings', 'islamicReminder');
    });
    // Register command: Show Prayer Times
    const showTimesCmd = vscode.commands.registerCommand('islamicReminder.showPrayerTimes', async () => {
        await showPrayerTimesInfo();
    });
    // Register command: Select Location
    const selectLocationCmd = vscode.commands.registerCommand('islamicReminder.selectLocation', async () => {
...
function startScheduler() {
    ...
    if (!config.cityId) {
        vscode.window.showWarningMessage('Pengingat Sholat: Silakan pilih lokasi Anda.', 'Pilih Lokasi').then((selection) => {
...
function getConfig() {
    ...
    return {
        enableReminder: config.get('enableReminder', true),
        enableSound: config.get('enableSound', true),
        enablePreReminder: config.get('enablePreReminder', true),
        cityId: config.get('cityId', '1301'),
        cityName: config.get('cityName', 'KOTA JAKARTA'),
        provinceName: config.get('provinceName', 'DKI JAKARTA'),
    };
}
// ============================================
// API FETCH
...
async function fetchAndCacheSchedule() {
    ...
    try {
        ...
        if (parsed.status && parsed.data && parsed.data.jadwal) {
            ...
            const cacheData = { date: today, timings, location: config.cityId };
        ...
        try {
            ...
            if (cached && cached.timings) {
                vscode.window.showWarningMessage('Pengingat Sholat: Gagal mengambil jadwal baru. Menggunakan data cache.');
                return cached.timings;
            }
        }
        catch (e) {
        ...
        vscode.window.showWarningMessage('Pengingat Sholat: Gagal mengambil jadwal sholat. Periksa koneksi internet Anda.');
        return null;
    }
}
...
async function checkPrayerTimes() {
    ...
    const lastDate = extensionContext.globalState.get('lastCheckDate');
    if (lastDate !== today) {
        triggeredPrayers.clear();
        triggeredPreReminders.clear();
    ...
    for (const prayer of PRAYER_KEYS) {
        ...
        if (config.enablePreReminder && !triggeredPreReminders.has(prayerKey)) {
            ...
            if (currentTime === preTime) {
                ...
                vscode.window.showInformationMessage(`🕌 ${PRAYER_DISPLAY_NAMES[prayer]} dalam 5 menit (${prayerTime})`);
            }
        }
...
// ============================================
async function showPrayerTimesInfo() {
    const schedule = await fetchAndCacheSchedule();
    if (!schedule) {
...
// WEBVIEW POPUP
// ============================================
function showPrayerPopup(prayer, time) {
    if (currentPanel) {
        try {
    ...
    currentPanel = vscode.window.createWebviewPanel('islamicPrayerReminder', `${displayName} Prayer Time`, vscode.ViewColumn.One, {
        enableScripts: true,
    ...
    const audioUri = currentPanel.webview.asWebviewUri(vscode.Uri.file(path.join(extensionContext.extensionPath, 'media', adzanFile)));
    const enableSound = config.enableSound;
    currentPanel.webview.html = getWebviewHtml(displayName, time, message, quote, audioUri.toString(), enableSound);
...
// WEBVIEW HTML
// ============================================
function getWebviewHtml(prayerName, time, message, quote, audioSrc, enableSound) {
    return /*html*/ `
<!DOCTYPE html>
<html lang="en">
<head>
...
async function loadCitiesData() {
    try {
        ...
        if (parsed.status && parsed.data && Array.isArray(parsed.data)) {
            allCities = parsed.data.map((city) => ({
                id: city.id,
                lokasi: city.lokasi,
            }));
...
function groupCitiesByProvince() {
    const provinceMap = new Map();
    for (const city of allCities) {
        const parts = city.lokasi.split(' ');
        let province = '';
        if (city.id.startsWith('01')) {
            province = 'ACEH';
        }
        else if (city.id.startsWith('02')) {
            province = 'SUMATERA UTARA';
        }
        else if (city.id.startsWith('03')) {
            province = 'SUMATERA BARAT';
        }
        else if (city.id.startsWith('04')) {
            province = 'RIAU';
        }
        else if (city.id.startsWith('05')) {
            province = 'KEPULAUAN RIAU';
        }
        else if (city.id.startsWith('06')) {
            province = 'JAMBI';
        }
        else if (city.id.startsWith('07')) {
            province = 'BENGKULU';
        }
        else if (city.id.startsWith('08')) {
            province = 'SUMATERA SELATAN';
        }
        else if (city.id.startsWith('09')) {
            province = 'BANGKA BELITUNG';
        }
        else if (city.id.startsWith('10')) {
            province = 'LAMPUNG';
        }
        else if (city.id.startsWith('11')) {
            province = 'BANTEN';
        }
        else if (city.id.startsWith('12')) {
            province = 'JAWA BARAT';
        }
        else if (city.id.startsWith('13')) {
            province = 'DKI JAKARTA';
        }
        else if (city.id.startsWith('14')) {
            province = 'JAWA TENGAH';
        }
        else if (city.id.startsWith('15')) {
            province = 'DI YOGYAKARTA';
        }
        else if (city.id.startsWith('16')) {
            province = 'JAWA TIMUR';
        }
        else if (city.id.startsWith('17')) {
            province = 'BALI';
        }
        else if (city.id.startsWith('18')) {
            province = 'NUSA TENGGARA BARAT';
        }
        else if (city.id.startsWith('19')) {
            province = 'NUSA TENGGARA TIMUR';
        }
        else if (city.id.startsWith('20')) {
            province = 'KALIMANTAN BARAT';
        }
        else if (city.id.startsWith('21')) {
            province = 'KALIMANTAN SELATAN';
        }
        else if (city.id.startsWith('22')) {
            province = 'KALIMANTAN TENGAH';
        }
        else if (city.id.startsWith('23')) {
            province = 'KALIMANTAN TIMUR';
        }
        else if (city.id.startsWith('24')) {
            province = 'KALIMANTAN UTARA';
        }
        else if (city.id.startsWith('25')) {
            province = 'GORONTALO';
        }
        else if (city.id.startsWith('26')) {
            province = 'SULAWESI SELATAN';
        }
        else if (city.id.startsWith('27')) {
            province = 'SULAWESI TENGGARA';
        }
        else if (city.id.startsWith('28')) {
            province = 'SULAWESI TENGAH';
        }
        else if (city.id.startsWith('29')) {
            province = 'SULAWESI UTARA';
        }
        else if (city.id.startsWith('30')) {
            province = 'SULAWESI BARAT';
        }
        else if (city.id.startsWith('31')) {
            province = 'MALUKU';
        }
        else if (city.id.startsWith('32')) {
            province = 'MALUKU UTARA';
        }
        else if (city.id.startsWith('33')) {
            province = 'PAPUA';
        }
        else if (city.id.startsWith('34')) {
...
// ============================================
async function selectLocationUI() {
    ...
    const provinces = Array.from(new Set(allCities.map(c => c.province).filter((p) => !!p)));
    provinces.sort();
    ...
    vscode.window.showInformationMessage(`Lokasi diatur: ${selectedCity.city.lokasi}, ${selectedProvince}`);
}
// ============================================
// STATUS BAR UPDATE
// ============================================
